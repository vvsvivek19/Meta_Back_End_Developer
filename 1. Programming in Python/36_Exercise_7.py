...
def to_mod_list(employee_list):
   ...
   employee_dept_object = map(mod,employee_list)
   employee_dept_list = []
   for x in employee_dept_object:
      employee_dept_list.append(x)
   return employee_dept_list
...
def generate_usernames(mod_list):
   ...
   """
   ### WRITE SOLUTION CODE HERE
   user_names = [x.replace(" ", "_") for x in mod_list]
   return user_names
   raise NotImplementedError()
...
def map_id_to_initial(employee_list):
   ...
   """
   ### WRITE SOLUTION CODE HERE
   ID_intial_dict = {employee['name'][0] : employee['id'] for employee  in employee_list}
   return ID_intial_dict
   raise NotImplementedError()
